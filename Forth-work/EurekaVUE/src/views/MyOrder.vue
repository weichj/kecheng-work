  ...
  methods: {
    getDate () {
      ...
      if (sessionStorage.getItem('identity') === 'user') {
        ...
        axios.get('http://localhost:8010/order/orders' + sessionStorage.getItem('username')).then(function (resp) {
          _this.tableDate = resp.data.data
        }).catch((error) => {
          console.log(error)
        })
      } else if (sessionStorage.getItem('identity') === 'admin') {
        axios.get('http://localhost:8010/order/orders').then(function (resp) {
          _this.tableDate = resp.data.data
        }).catch((error) => {
          console.log(error)
        })
    ...
    getStateDate () {
      const _this = this
      axios.get('http://localhost:8010/order/orders/State').then(function (resp) {
        _this.tableDate = resp.data.data
      }).catch((error) => {
        console.log(error)
      })
    ...
    DeleteOrder (Num) {
      const _this = this
      axios.delete('http://localhost:8010/order/orders/' + Num).then(function (resp) {
